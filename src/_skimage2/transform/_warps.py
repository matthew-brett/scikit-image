...
def rotate(
    ...
    if center is None:
        center = np.array((rows, cols)) / 2.0 - 0.5
    else:
    ...
    if resize:
        ...
        corners = np.array([[0, 0], [rows - 1, 0], [rows - 1, cols - 1], [0, cols - 1]])
...
def warp_coords(coord_map, shape, dtype=np.float64):
    ...
    Examples
    --------
    Produce a coordinate map that shifts an image down and to the right:

    >>> from skimage import data
    >>> from scipy.ndimage import map_coordinates
    >>>
    >>> def shift_down10_right20(ij):
    ...     return ij - np.array([10, 20])[None, :]
    >>>
    >>> image = data.astronaut().astype(np.float32)
    >>> coords = warp_coords(shift_down10_right20, image.shape)
    ...
    tf_coords = np.indices((rows, cols), dtype=dtype).reshape(2, -1).T
    ...
    tf_coords = tf_coords.T.reshape((-1, rows, cols))

    # Place the row-coordinate mapping
    _stackcopy(coords[0, ...], tf_coords[0, ...])

    # Place the col-coordinate mapping
    _stackcopy(coords[1, ...], tf_coords[1, ...])
    ...
    inverse_map : transformation object, callable ``cr = f(cr, **kwargs)``, or ndarray
        ...
        information.
         ...
         - For 2-D images, a function that transforms a ``(M, 2)`` array of
           ``(row, col)`` coordinates in the output image to their
