...
from skimage._shared.compat import NP_COPY_IF_NEEDED
...
class AffineTransform(ProjectiveTransform):
    ...
    Has the following form::
        ...
        I = a0 * i + a1 * j + a2
          =   si * i * [cos(rotation) + tan(shear_j) * sin(rotation)]
            - sj * j * [tan(shear_i) * cos(rotation) + sin(rotation)]
            + translation_i

        J = b0 * i + b1 * j + b2
          =   si * i * [sin(rotation) - tan(shear_j) * cos(rotation)]
            - sj * j * [tan(shear_i) * sin(rotation) - cos(rotation)]
            + translation_j

    where ``si`` and ``sj`` are scale factors in the i and j directions.
    ...
    rotation, shear, and translation in i (a2) and j (b2). For 3D and higher,
    ...
    scale : {s as float or (si, sj) as array, list or tuple}, optional
        Scale factor(s). If a single value, it will be assigned to both
        si and sj. Only available for 2D.

        .. versionadded:: 0.17
           Added support for supplying a single scalar value.
    shear : float or 2-tuple of float, optional
        The i and j shear angles, clockwise, by which these axes are
        rotated around the origin [2].
        If a single value is given, take that to be the i shear angle, with
        the j angle remaining 0. Only available in 2D.
    rotation : float, optional
        Rotation angle, clockwise, as radians. Only available for 2D.
    translation : (ti, tj) as array, list or tuple, optional
    ...
    def _srst2matrix(self, scale, rotation, shear, translation):
        ...
        si, sj = (scale, scale) if np.isscalar(scale) else scale
        ...
        shear_i, shear_j = (shear, 0) if np.isscalar(shear) else shear
        ...
        a0 = si * (math.cos(rotation) + math.tan(shear_j) * math.sin(rotation))
        a1 = -sj * (math.tan(shear_i) * math.cos(rotation) + math.sin(rotation))

        b0 = si * (math.sin(rotation) - math.tan(shear_j) * math.cos(rotation))
        b1 = -sj * (math.tan(shear_i) * math.sin(rotation) - math.cos(rotation))
...
def _euler_rotation_matrix(angles, degrees=False):
    ...
    return spatial.transform.Rotation.from_euler(
        'IJK', angles=angles, degrees=degrees
    ).as_matrix()
...
class EuclideanTransform(ProjectiveTransform):
    ...
    Has the following form::
        ...
        I = a0 * i - b0 * j + a1 =
          = i * cos(rotation) - j * sin(rotation) + a1

        J = b0 * i + a0 * j + b1 =
          = i * sin(rotation) + j * cos(rotation) + b1
    ...
    translation along `i`, `b1` along `j`, etc.). Beyond 3D, if the
    ...
    translation : (i, j[, k, ...]) sequence of float, length D, optional
...
class SimilarityTransform(EuclideanTransform):
    ...
    Has the following form in 2D::
        ...
        I = a0 * i - b0 * j + a1 =
          = s * i * cos(rotation) - s * j * sin(rotation) + a1

        J = b0 * i + a0 * j + b1 =
          = s * i * sin(rotation) + s * j * cos(rotation) + b1
    ...
    rotation : float, optional
        ...
        Implemented only for 2D and 3D. For 3D, this is given in IJK Euler
        angles.
    translation : array_like of shape (D,), optional
        i, j[, k] translation parameters. Implemented only for 2D and 3D.
    dimensionality : int, optional
...
class PolynomialTransform(_GeometricTransform):
    ...
    Has the following form::
        ...
        I = sum[k=0:order]( sum[m=0:k]( a_km * i**(k - m) * j**m ))
        J = sum[k=0:order]( sum[m=0:k]( b_km * i**(k - m) * j**m ))

    Parameters
    ----------
    params : array_like of shape (2, N), optional
        Polynomial coefficients where `N * 2 = (order + 1) * (order + 2)`. So,
        a_km is defined in `params[0, :]` and b_km in `params[1, :]`.
    dimensionality : int, optional
    ...
    def from_estimate(cls, src, dst, order=2, weights=None):
        ...
        These equations can be transformed to the following form::
            ...
            0 = sum[k=0:order]( sum[m=0:k]( a_km * i**(k - m) * j**m )) - I
            0 = sum[k=0:order]( sum[m=0:k]( b_km * i**(k - m) * j**m )) - J
